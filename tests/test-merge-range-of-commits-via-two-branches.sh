...
cd git-svn-project
	...
	git co feature
		git svn merge `git rev-list --reverse trunk -2`
		git log -1
		git svn dcommit
		git svn propget svn:mergeinfo
} > $BASE_LOG_DIR/$0.log 2>&1
		assert_equals `git svn propget svn:mergeinfo` "/svn-project/trunk:8,9"

{
	git co feature_2
		...
		assert_contains "`git svn propget svn:mergeinfo`" "/svn-project/trunk:8-9"
